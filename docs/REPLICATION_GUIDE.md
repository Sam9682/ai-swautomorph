...
### Test Connectivity
```bash
python3 scripts/replication_cli.py test 192.168.1.11
```
...
# Test manual sync
python3 scripts/replication_cli.py sync users <peer_ip>
...
for table in users applications user_applications billing_activities sso_tokens; do
    python3 scripts/replication_cli.py sync $table <peer_ip>
done
```
