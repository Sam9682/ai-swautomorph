...
case $COMMAND in
    "ps")
        ...
        if command -v docker-compose &> /dev/null; then
            PORT=$((PORT_RANGE_BEGIN + USER_ID)) HTTPS_PORT=$((PORT_RANGE_BEGIN + USER_ID + 363)) USER_ID=$USER_ID docker-compose ps
        else
    ...
    "stop")
        ...
        PORT=$((PORT_RANGE_BEGIN + USER_ID)) HTTPS_PORT=$((PORT_RANGE_BEGIN + USER_ID + 363)) USER_ID=$USER_ID docker-compose down
        echo "✅ Services stopped"
        exit 0
        ;;
    "logs")
        PORT=$((PORT_RANGE_BEGIN + USER_ID)) HTTPS_PORT=$((PORT_RANGE_BEGIN + USER_ID + 363)) USER_ID=$USER_ID docker-compose logs -f
        exit 0
        ;;
    "restart")
        echo "🔄 Restarting AI-SwAutoMorph services..."
        PORT=$((PORT_RANGE_BEGIN + USER_ID)) HTTPS_PORT=$((PORT_RANGE_BEGIN + USER_ID + 363)) USER_ID=$USER_ID docker-compose restart
...
echo "🧹 Cleaning up..."
PORT=$((PORT_RANGE_BEGIN + USER_ID)) HTTPS_PORT=$((PORT_RANGE_BEGIN + USER_ID + 363)) USER_ID=$USER_ID docker-compose down --remove-orphans 2>/dev/null || true

# Build and start services
echo "🔨 Building Docker images..."
PORT=$((PORT_RANGE_BEGIN + USER_ID)) HTTPS_PORT=$((PORT_RANGE_BEGIN + USER_ID + 363)) USER_ID=$USER_ID docker-compose build --no-cache

echo "🚀 Starting services..."
PORT=$((PORT_RANGE_BEGIN + USER_ID)) HTTPS_PORT=$((PORT_RANGE_BEGIN + USER_ID + 363)) USER_ID=$USER_ID docker-compose up -d
